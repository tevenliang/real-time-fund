...
CREATE INDEX IF NOT EXISTS idx_user_configs_ytd_return_rate ON public.user_configs (ytd_return_rate);
...
GRANT EXECUTE ON FUNCTION public.update_user_config_full(jsonb, text, boolean) TO anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_ytd_percentile(numeric) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_ytd_percentile(numeric) TO service_role;

GRANT EXECUTE ON FUNCTION public.update_user_config_partial(jsonb, text, boolean) TO anon, authenticated;
...
-- 关联板块表 (fund_related)
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.fund_related (
  id bigint generated by default as identity not null,
  created_at timestamp with time zone not null default now(),
...
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.fund_secid (
  id bigint generated by default as identity not null,
  created_at timestamp with time zone not null default now(),
  related_sector text null,
...
-- 基金主题表 (fund_topic)
-- --------------------------------------------------------
CREATE TABLE IF NOT EXISTS public.fund_topic (
  id bigint generated by default as identity not null,
  created_at timestamp with time zone not null default now(),
...
-- 索引加速查询
CREATE INDEX IF NOT EXISTS idx_ocr_daily_usage_user_date ON public.ocr_daily_usage (user_id, usage_date);
...
GRANT EXECUTE ON FUNCTION public.check_and_increment_ocr_usage(int) TO anon, authenticated;
