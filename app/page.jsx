...
export default function HomePage() {
  ...
  const displayFundsRaw = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (sortBy === 'researchSignal') {
        const sa = a.research?.signal?.score;
        const sb = b.research?.signal?.score;
        const hasA = isNumber(sa);
        const hasB = isNumber(sb);
        if (!hasA && !hasB) return 0;
        if (!hasA) return 1;
        if (!hasB) return -1;
        return sortOrder === 'asc' ? sa - sb : sb - sa;
      }
      if (sortBy === 'researchCagr') {
        const get = (f) => researchByCode[f.code]?.data?.risk?.cagr;
        const va = get(a);
        const vb = get(b);
        const hasA = isNumber(va);
        const hasB = isNumber(vb);
        if (!hasA && !hasB) return 0;
        if (!hasA) return 1;
        if (!hasB) return -1;
        return sortOrder === 'asc' ? va - vb : vb - va;
      }
      if (sortBy === 'researchMdd') {
        const get = (f) => researchByCode[f.code]?.data?.risk?.mdd;
        const va = get(a);
        const vb = get(b);
        const hasA = isNumber(va);
        const hasB = isNumber(vb);
        ...
        if (!hasA) return 1;
        if (!hasB) return -1;
        // 回撤恒负：asc 升序=绝对值小的在前（回撤小）；desc 反之
        return sortOrder === 'asc' ? va - vb : vb - va;
      }
      if (sortBy === 'researchVol') {
        const get = (f) => researchByCode[f.code]?.data?.risk?.vol;
        const va = get(a);
        const vb = get(b);
        const hasA = isNumber(va);
        const hasB = isNumber(vb);
      ...
      if (sortBy === 'researchSharpe') {
        const get = (f) => researchByCode[f.code]?.data?.risk?.sharpe;
        const va = get(a);
        const vb = get(b);
        const hasA = isNumber(va);
        const hasB = isNumber(vb);
