...
import { withRetry, asyncPool } from '../lib/asyncHelper';
...
// allFund.json cache for fund name fallback
let _allFundNameCache = null;
let _allFundLoadPromise = null;

const getFundNameFromLocalList = async (code) => {
  if (_allFundNameCache && _allFundNameCache.has(code)) {
    return _allFundNameCache.get(code);
  }
  if (!_allFundLoadPromise) {
    _allFundLoadPromise = fetch('/allFund.json')
      .then((r) => r.json())
      .then((list) => {
        _allFundNameCache = new Map(list.map((f) => [f.code, f.name]));
        _allFundLoadPromise = null;
      })
      .catch(() => { _allFundLoadPromise = null; });
  }
  await _allFundLoadPromise;
    ...
    try {
      ...
      const namePromise = (async () => {
        try {
          ...
          const found = results.find((item) => item.CODE === c && item.CATEGORY !== 150);
...
export const fetchQdiiValuationFromSupabase = async (code) => {
  ...
  // 主路径：查 Supabase gs_qdii 表（由后台脚本定期填充）
  if (isSupabaseConfigured) {
    try {
      ...
      if (!error && data) {
        const gszzlVal = data.gszzl != null && Number.isFinite(Number(data.gszzl)) ? Number(data.gszzl) : null;
        return {
  ...
  // 回退：尝试天天基金 API（部分 QDII 可能有效）
  try {
    const url = `https://fundcomapi.tiantianfunds.com/mm/newCore/FundValuationLast?FCODES=${encodeURIComponent(normalized)}&FIELDS=SHORTNAME,FCODE,DWXJZ,DWJZ,GSZ,GSSJ,PDATETIME,GSDATE`;
    const resp = await fetch(url);
    if (resp.ok) {
      const json = await resp.json();
      const item = Array.isArray(json?.data) ? json.data.find(d => d.FCODE === normalized) : null;
      if (item && item.GSZ != null && Number.isFinite(Number(item.GSZ))) {
        return {
          gztime: item.GSSJ || item.PDATETIME || null,
          gszzl: null,
          valuationSource: 'tiantian_qdii_fallback',
          gzstatus: item.GSSJ ? '估' : null
        };
      }
    }
  } catch (e) {
  ...
  try {
    return await qc.fetchQuery({
      ...
      queryFn: async () => {
        ...
        try {
          const name = await getFundNameFromLocalList(normalized);
          return typeof name === 'string' && name.includes('(QDII)');
        } catch {
          return false;
        }
      },
...
export async function fetchBestValuationSource(code, jzrq, actualZzl) {
  ...
  if (isSupabaseConfigured && supabase?.functions?.invoke) {
    try {
      ...
      // edge function 不存在或失败，降级到客户端计算
    }
  }

  // 客户端降级：用历史精度表 RPC 获取各源误差，或直接返回 null（让 UI 不显示精度标签）
  if (!isSupabaseConfigured) return null;
  try {
    const { data } = await supabase.rpc('get_fund_best_source', { p_fund_code: c });
    const sourceName = Array.isArray(data) ? data[0]?.source : data?.source;
    const bestId = SOURCE_NAME_TO_ID[sourceName] ?? null;
    // 返回基本信息（精度标签会在前端用实时误差补充）
    const res = { bestSource: bestId, isYesterdayAccuracy: true, isTodayAccuracy: false, diffs: {} };
    qc.setQueryData(cacheKey, res, { staleTime: 30 * 60 * 1000 });
    return res;
  } catch (_) {
    return null;
  }
...
export async function fetchFundsBestSources(fundCodes) {
  ...
  // 对每只未缓存的基金并行调用 fetchFundBestSource（含实时数据 fallback 逻辑）
  // 若历史最优源此刻无实时数据，会自动降到有数据的源
  // 限制并发数（4），避免同时注入过多新浪 JSONP 触发浏览器并发限制/新浪限流，
  // 导致 v2/v3 拿不到实时数据而错误回退到无数据的历史最优源（数据源1）
  try {
    const bestIds = await asyncPool(3, missingCodes, (code) =>
      fetchFundBestSource(code).catch(() => null)
    );
    bestIds.forEach((val, idx) => {
...
export const fetchFundData = async (c, overrideDataSource) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if (!baseData.name) {
      // 占位符判断：基金(6位数字) 格式 -> 强制重新查询
      const isPlaceholder = storedName && /^基金\(\d{6}\)$/.test(storedName);
      if (storedName && !isPlaceholder) {
        baseData.name = storedName;
      } else {
        try {
          const results = await searchFunds(code);
          const found = results.find((item) => item.CODE === code && item.CATEGORY !== 150);
          if (found && (found.NAME || found.SHORTNAME)) {
            baseData.name = found.NAME || found.SHORTNAME;
          } else if (isPlaceholder) {
            const localName = await getFundNameFromLocalList(code);
            if (localName) baseData.name = localName;
          }
        } catch (e) {
          if (isPlaceholder) {
            try {
