...
export default function LoginModal({ onClose, showToast, isExplicitLoginRef, initialError = '' }) {
  ...
  const handlePasswordLogin = async () => {
    ...
    // 直接 fetch 调用，绕开 supabase-js 可能的网络问题；带 15s 超时保护
    ...
    try {
      ...
      if (!res.ok) {
        const msg = payload?.msg || payload?.error_description || ('HTTP ' + res.status);
      ...
      // 拿到 access_token 和 refresh_token 后手动设置 session，让 supabase-js 后续能正常使用
      if (payload?.access_token && payload?.refresh_token && payload?.user) {
        try {
          ...
          // 即便 setSession 失败，也把 token 存到 localStorage，让 supabase-js 之后能识别
          try {
        ...
        setLoginError('登录响应格式异常');
        if (isExplicitLoginRef) isExplicitLoginRef.current = false;
      }
    } catch (err) {
      clearTimeout(timeoutId);
      if (err.name === 'AbortError') {
        setLoginError('请求超时，请检查网络后重试');
      ...
      <div className="modal-overlay" role="dialog" aria-modal="true" aria-label="登录" onClick={onClose}>
        <div ref={loginModalCardRef} className="glass card modal login-modal" onClick={(e) => e.stopPropagation()}>
          <div className="title" style={{ marginBottom: 16 }}>
            ...
            <span>邮箱登录</span>
            <span className="muted">{loginMode === 'password' ? '使用邮箱密码登录' : '使用邮箱验证登录'}</span>
          </div>

          <form onSubmit={handleSendOtp}>
            <div className="form-group" style={{ marginBottom: 16 }}>
              <div className="muted" style={{ marginBottom: 8, fontSize: '0.8rem' }}>
                {loginMode === 'password' ? '请输入已注册邮箱与密码' : '请输入邮箱，我们将发送验证码到您的邮箱'}
              </div>
              <input
                style={{ width: '100%' }}
                className="input"
            ...
            <div className="row" style={{ justifyContent: 'space-between', gap: 12, alignItems: 'center', marginBottom: 12 }}>
              <button
                type="button"
              ...
              >
                {loginMode === 'otp' ? '使用密码登录' : '使用邮箱验证码登录'}
              </button>
            </div>
            <div className="row" style={{ justifyContent: 'flex-end', gap: 12 }}>
              ...
              >
                {loginLoading ? '处理中...' : (loginMode === 'password' ? '登录' : (loginSuccess ? '确认验证码' : '发送邮箱验证码'))}
              </button>
            </div>
          </form>
