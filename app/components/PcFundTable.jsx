...
const PcFundTable = memo(function PcFundTable({
  ...
  // 基金研究数据（波段/风险指标）—— 仅当任一研究列可见时才批量拉取
  const researchColumnsEnabled =
    columnVisibility?.researchSignal !== false ||
    columnVisibility?.researchCagr !== false ||
    columnVisibility?.researchMdd !== false ||
    columnVisibility?.researchVol !== false ||
    columnVisibility?.researchSharpe !== false;
  const researchCodes = useMemo(
    () => (researchColumnsEnabled ? (data || []).map((f) => f.code).filter(Boolean) : []),
    [researchColumnsEnabled, data]
  );
  const researchMap = useFundResearchBatch(researchCodes);
  ...
  const columns = useMemo(
    () => [
      ...
      {
        accessorKey: 'researchSignal',
        header: '波段信号',
        size: 130,
        minSize: 100,
        cell: (info) => {
          const original = info.row.original || {};
          const research = original.research;
          const loading = original.researchLoading;
          if (loading && !research) {
            return <div className="muted" style={{ fontSize: 'clamp(10px, 1.2vw, 13px)' }}>…</div>;
          }
          const sig = research?.signal;
          if (!sig || sig.score == null) {
            return <div className="muted" style={{ fontSize: 'clamp(10px, 1.2vw, 13px)' }}>—</div>;
          }
          const color = sig.zone_color || 'var(--primary)';
          const score = Number(sig.score).toFixed(1);
          const zone = sig.zone || '';
          return (
            <Tooltip>
              <TooltipTrigger asChild>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 0 }}>
                  ...
                  <span style={{ fontSize: '11px', color }}>{zone}</span>
                </div>
              </TooltipTrigger>
              <TooltipContent>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: '12px' }}>
                  <div>建议：{sig.advice || '—'}</div>
                  <div className="muted">250日分位 {sig.pct} · RSI14 {sig.rsi} · BIAS20 {sig.bias}</div>
                </div>
              </TooltipContent>
            </Tooltip>
          );
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const v = research?.risk?.cagr;
          if (v == null) return <div className="muted" style={{ fontSize: 'clamp(10px, 1.2vw, 13px)' }}>—</div>;
          const n = Number(v);
          const cls = n > 0 ? 'up' : n < 0 ? 'down' : 'muted';
          const text = `${n > 0 ? '+' : ''}${n.toFixed(2)}%`;
          return (
            <div className={cls} style={{ fontWeight: 700, fontSize: 'clamp(10px, 1.2vw, 14px)', textAlign: 'right' }}>
              {text}
            </div>
          );
        },
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const v = research?.risk?.sharpe;
          if (v == null) return <div className="muted" style={{ fontSize: 'clamp(10px, 1.2vw, 13px)' }}>—</div>;
          const n = Number(v);
          const color = n >= 1 ? '#874EA9' : n >= 0 ? 'var(--muted-foreground)' : '#1e8449';
          return (
            <div
              style={{
                fontWeight: 700,
  ...
  // 把基金研究数据（波段/风险）merge 到行数据，供 cell 回调读取
  const enrichedData = useMemo(() => {
    if (!researchColumnsEnabled) return data;
    return (data || []).map((f) => {
      const r = researchMap[f.code];
      return { ...f, research: r?.data, researchLoading: !r?.data && r?.isLoading };
    });
  }, [data, researchMap, researchColumnsEnabled]);
...
function FundDetailDialog({ blockDialogClose, cardDialogRow, getFundCardProps, setCardDialogRow }) {
  ...
  return (
    ...
    >
      <DialogContent
        className="sm:max-w-5xl max-h-[88vh] flex flex-col p-0 overflow-hidden"
