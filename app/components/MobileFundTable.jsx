...
const MobileFundTable = memo(function MobileFundTable({
    ...
    () => [
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          if (loading && !research) {
            return <span className="muted" style={{ fontWeight: 700 }}>…</span>;
          }
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 0 }}>
              <span style={{ fontWeight: 700, color: sig.zone_color || 'var(--primary)', fontSize: 'clamp(10px, 3vw, 14px)' }}>
                {Number(sig.score).toFixed(1)}
              </span>
              <span style={{ fontSize: '10px', color: sig.zone_color || 'var(--primary)' }}>{sig.zone || ''}</span>
            </div>
          );
        },
        meta: { align: 'right', cellClassName: 'value-cell', width: columnWidthMap.researchSignal }
      },
      {
        ...
        cell: (info) => {
          ...
          if (v == null) return <span className="muted" style={{ fontWeight: 700 }}>—</span>;
          const n = Number(v);
          const color = n >= 1 ? '#874EA9' : n >= 0 ? 'var(--muted-foreground)' : '#1e8449';
          return (
            <span style={{ fontWeight: 700, color, fontSize: 'clamp(10px, 3vw, 14px)' }}>
              {n.toFixed(2)}
            </span>
          );
        },
        meta: { align: 'right', cellClassName: 'change-cell', width: columnWidthMap.researchSharpe }
      },
      {
  ...
  // 基金研究数据（移动端只关心波段+夏普 2 列）
  const mobileResearchColumnsEnabled =
    (mobileColumnVisibility?.researchSignal ?? true) !== false ||
    (mobileColumnVisibility?.researchSharpe ?? true) !== false;
  const _mobileCodes = useMemo(
    () => (mobileResearchColumnsEnabled ? (data || []).map((f) => f?.code).filter(Boolean) : []),
    [mobileResearchColumnsEnabled, data]
  );
  const mobileResearchMap = useFundResearchBatch(_mobileCodes);

  // 把研究数据 merge 到行数据
  const enrichedData = useMemo(() => {
    if (!mobileResearchColumnsEnabled) return data;
    return (data || []).map((f) => {
      const r = mobileResearchMap[f.code];
      return { ...f, research: r?.data, researchLoading: !r?.data && r?.isLoading };
    });
  }, [data, mobileResearchMap, mobileResearchColumnsEnabled]);
