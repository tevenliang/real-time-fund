  ...
  const refreshAll = useCallback(
    async (codes) => {
      ...
      try {
        ...
        // 所有基金都自动选最优源（有实时数据的最准的那个）
        let bestSourcesMap = {};
        try {
          if (uniqueCodes.length > 0) {
            bestSourcesMap = await fetchFundsBestSources(uniqueCodes);
            // 立即将最优源写回 storage（下次刷新直接用缓存，不再重复调用 RPC）
            const stored = useStorageStore.getState().funds || [];
            const needsUpdate = stored.some(
              (f) =>
                uniqueCodes.includes(f.code) &&
            ...
            if (needsUpdate) {
              useStorageStore.getState().setFunds((prev) => {
                const changed = prev.map((f) => {
                  const newDs = bestSourcesMap[f.code];
                  if (newDs && (f.dataSource !== newDs || f.autoSource !== true)) {
                    return { ...f, dataSource: newDs, autoSource: true };
                  }
                  return f;
                });
                return changed.some((f, i) => f !== prev[i]) ? changed : prev;
              });
            }
          }
        } catch (e) {
