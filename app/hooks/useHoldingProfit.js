...
export function useHoldingProfit({ activeGroupId } = {}) {
  ...
  const getHoldingProfit = useCallback(
    (fund, holding, scopeGroupIdOverride) => {
      ...
      const isQdiiFund = fund.valuationSource === 'supabase_qdii';
      // QDII 基金：接受最近1个交易日内的估值（东方财富 lsfund 数据 T+1）
      let hasTodayValuation = isString(fund.gztime) && fund.gztime.startsWith(todayStr);
      if (!hasTodayValuation && isQdiiFund && isString(fund.gztime)) {
        // 用 dayjs 判断 gztime 是否在今天或昨天的本地日期范围内
        // gztime 格式：2026-08-17T00:00:00+00:00，取前半段 YYYY-MM-DD 比较
        const gzDateStr = fund.gztime.substring(0, 10);
        const yesterday = toTz(todayStr).subtract(1, 'day').format('YYYY-MM-DD');
        hasTodayValuation = gzDateStr === todayStr || gzDateStr === yesterday;
      }
